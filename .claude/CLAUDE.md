...
DXT (pronounced "dee-ex-tee") is a lightweight Extract-Load tool for building simple, reliable batch data movement pipelines. It aims to be a YAML-first, CLI-driven alternative to writing custom Python scripts or deploying full ELT platforms.
...
4. DXT is installed in editable mode and available as the `dxt` command
...
Based on the README, DXT will be structured around:
...
### DXT CLI Commands
```bash
dxt --version              # Show version
dxt --help                 # Show help
dxt run <pipeline.yaml>    # Run a pipeline (not yet implemented)
dxt validate <pipeline.yaml>  # Validate a pipeline (not yet implemented)
dxt init [name]            # Create a pipeline template (not yet implemented)
```

### Development Tools
```bash
pytest                     # Run tests
pytest --cov=dxt          # Run tests with coverage
